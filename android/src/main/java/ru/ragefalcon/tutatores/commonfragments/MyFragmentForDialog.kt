...
open class MyFragmentForDialogVM<T : ViewBinding>(inflateBF: (LayoutInflater, ViewGroup?, Boolean) -> T) :
    MyFragmentForDialog<T>(inflateBF) {
...
open class MyFragmentForDialog<T : ViewBinding>(inflateBF: (LayoutInflater, ViewGroup?, Boolean) -> T) :
    BaseFragment<T>(inflateBF) {
    var dismissDialValue: ((() -> Unit)?) -> Unit = {}
    fun dismissDial() {
        dismissDialValue(null)
    }

    fun dismissDial(endF: (() -> Unit)) {
        dismissDialValue(endF)
    }
