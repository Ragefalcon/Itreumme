...
class PlanTabAnim() {
    ...
    val listStateStap = MyLazyListState()
    ...
    private fun ColumnScope.dropMenuPlanStapItem(
        ...
        with(LocalDensity.current) {
            Row {
                for (cc in 0..4) {
                    MyIconSimpleButt(
                        "bookmark_01.svg",
                        MySelectStat.statNabor3.find { it.first == cc.toLong() }?.second ?: Color(0xFFFFF42B)
                    ) {
        ...
        if (item.stat != TypeStatPlanStap.COMPLETE) MyDropdownMenuItem(
            expanded, itemPlanStapStyle.dropdown, "+ Подэтап"
        ) {
            selection.selected?.let { itemPlan ->
            ...
            if (item.summa == null) MyDropdownMenuItem(expanded, itemPlanStapStyle.dropdown, "Добавить фин. цель") {
                MyEmptyPanel(dialLay, false) { dialInner, closeFun ->
                    ...
                    MyTextButtStyle1("Создать новый счет-план") {
                        closeFun()
                        PanAddSchetPlan(
                            dialLay, null, item.name, Pair(TypeBindElementForSchetPlan.PLANSTAP, item.id.toLong())
                        )
                    }
                    MyTextButtStyle1("Отмена") {
                        closeFun()
                ...
                MyEmptyPanel(dialLay, false) { dialInner, closeFun ->
                    if (item.schplOpen == true) MyTextButtStyle1("Удалить/закрыть счет-план") {
                        MainDB.addFinFun.delBindWithDelOrCloseSchetPlan(
                            TypeBindElementForSchetPlan.PLANSTAP, item.id.toLong()
                        )
                        closeFun()
                    }
        ...
        fun checkSchetPlanStap(itemInner: ItemPlanStap) {
            MyEmptyPanel(dialLay, false) { dialInner, closeFun ->
                ...
                MyTextButtStyle1("Удалить/закрыть счет-план") {
                    MainDB.addFinFun.delBindWithDelOrCloseSchetPlan(
                        TypeBindElementForSchetPlan.PLANSTAP, itemInner.id.toLong()
                    )
                    closeFun()
                }
        ...
        MyDropdownMenuItem(
            ...
            MainDB.addTime.updStatPlanStap(
                item, if (item.stat != TypeStatPlanStap.FREEZE) TypeStatPlanStap.FREEZE else TypeStatPlanStap.VISIB
            )
        }
        ...
        MyDropdownMenuItem(
            expanded, itemPlanStapStyle.dropdown, if (item.stat != TypeStatPlanStap.CLOSE) "Закрыть" else "Открыть"
        ) {
            MainDB.addTime.updStatPlanStap(
                item, if (item.stat != TypeStatPlanStap.CLOSE) TypeStatPlanStap.CLOSE else TypeStatPlanStap.VISIB
            )
        ...
        MyCompleteDropdownMenuButton(expanded, item.stat == TypeStatPlanStap.COMPLETE) {
            MainDB.addTime.updStatPlanStap(
                item, if (item.stat != TypeStatPlanStap.COMPLETE) TypeStatPlanStap.COMPLETE else TypeStatPlanStap.VISIB
            )
        ...
        MyDeleteDropdownMenuButton(expanded) {
            ...
            if (item.quest_key_id == 0L) {
                ...
                MyShowMessage(
                    dialLay, "Этот элемент из квеста, его можно удалить только вместе с квестом."
                )
            }
        }
    ...
    private fun ColumnScope.dropMenuPlanItem(
            ...
            if (item.summa == null) MyDropdownMenuItem(expanded, itemPlanStyle.dropdown, "Добавить фин. цель") {
                MyEmptyPanel(dialLay, false) { dialInner, closeFun ->
                    ...
                    MyTextButtStyle1("Создать новый счет-план") {
                        closeFun()
                        PanAddSchetPlan(
                            dialLay, null, item.name, Pair(TypeBindElementForSchetPlan.PLAN, item.id.toLong())
                        )
                    }
                    MyTextButtStyle1("Отмена") {
                        closeFun()
                ...
                MyEmptyPanel(dialLay, false) { dialInner, closeFun ->
                    if (item.schplOpen == true) MyTextButtStyle1("Удалить/закрыть счет-план") {
                        MainDB.addFinFun.delBindWithDelOrCloseSchetPlan(
                            TypeBindElementForSchetPlan.PLAN, item.id.toLong()
                        )
                        closeFun()
                    }
                    MyTextButtStyle1("Только отвязать счет-план") {
        ...
        fun checkSchetPlan(itemInner: ItemPlan) {
            MyEmptyPanel(dialLay, false) { dialInner, closeFun ->
                ...
                MyTextButtStyle1("Удалить/закрыть счет-план") {
                    MainDB.addFinFun.delBindWithDelOrCloseSchetPlan(
                        TypeBindElementForSchetPlan.PLAN, itemInner.id.toLong()
                    )
                    closeFun()
                }
                MyTextButtStyle1("Отвязать счет-план") {
        ...
        MyDropdownMenuItem(
            expanded, itemPlanStyle.dropdown, if (item.stat != TypeStatPlan.FREEZE) "Заморозить" else "Разморозить"
        ) {
            MainDB.addTime.updStatPlan(
                item, if (item.stat != TypeStatPlan.FREEZE) TypeStatPlan.FREEZE else TypeStatPlan.VISIB
            )
        }

        MyDropdownMenuItem(
            expanded, itemPlanStyle.dropdown, if (item.stat != TypeStatPlan.CLOSE) "Закрыть" else "Открыть"
        ) {
            MainDB.addTime.updStatPlan(
                item, if (item.stat != TypeStatPlan.CLOSE) TypeStatPlan.CLOSE else TypeStatPlan.VISIB
            )
        ...
        if (item.direction.not()) MyCompleteDropdownMenuButton(expanded, item.stat == TypeStatPlan.COMPLETE) {
            MainDB.addTime.updStatPlan(
                item, if (item.stat != TypeStatPlan.COMPLETE) TypeStatPlan.COMPLETE else TypeStatPlan.VISIB
            )
            if (!vypPlan.value) selection.selected = null
    ...
    val planAnimPanel = CommonOpenItemPanel<ItemPlan>(whenOpen = {
        regexStr = ""
        ...
        MainDB.styleParam.timeParam.planTab.itemPlan.getComposable(::ItemPlanStyleState) { itemPlanStyle ->
            MyList(MainDB.timeSpis.spisPlan.getState().sortedBy { it.stat == TypeStatPlan.FREEZE } ?: listOf(),
                modifier = Modifier.padding(horizontal = 10.dp).padding(top = 5.dp, bottom = 5.dp).weight(1f)
                    .then(modifierList),
                lazyListState) { ind, itemPlan ->
                if (itemPlan.stat != TypeStatPlan.INVIS) {
                    ...
                    ) else ComItemPlan(itemPlan, selection, selFun = {
                    }, onDoubleClick = {
                    ...
                    }, itemPlanStyleState = itemPlanStyle, dialLay = dialLay, sortEnable = sortPlanEnable.value
                    ) { item, expanded, setDissFun ->
                        if (dialLay != null) dropMenuPlanItem(
                            item, selection, dialLay, expanded, itemPlanStyle, setDissFun
                        )
                    }.getComposable()
                }
            }
                ...
                if (startOpenAnimation.value) {
                    MainDB.styleParam.timeParam.planTab.itemPlanStap.getComposable(::ItemPlanStapStyleState) { itemPlanStapStyle ->
                        MainDB.timeSpis.spisPlanStap.getState().let { spisStap ->
                            ...
                            MyList(
                                spisStap,
                                Modifier.padding(horizontal = 0.dp, vertical = 5.dp).weight(1f).padding(bottom = 10.dp),
                                ...
                                if (itemPlanStap.stat != TypeStatPlanStap.INVIS && (regexStr == "" || regexStr.toRegex()
                                    ...
                                    if (itemPlanStap.stat == TypeStatPlanStap.BLOCK) ComItemPlanStapPlate(
                                        ...
                                        sverFun = {
                                            ...
                                            scopeR.run {
                                                MainDB.timeFun.setExpandStapPlan(
                                                    it.id.toLong(), svernutNew
                                                )
                                            }
                                        },
                                        changeGotov = { item, progress ->
                                            MainDB.timeSpis.spisPlanStap.updateElem(
                                                item, item.copy(gotov = progress.toDouble() * 100)
                                            )
                                            scopeR.run {
                                                MainDB.addTime.updGotovPlanStap(
                                                    item.id.toLong(), progress.toDouble() * 100
                                                )
                                            }
                                        },
                                        ...
                                        if (dialLay != null) dropMenuPlanStapItem(
                                            item, selection, dialLay, expanded, itemPlanStapStyle
                                        )
                                    }.getComposable()
                                }
                            }
    ...
    fun show(dialLay: MyDialogLayout) {
        ...
        Column(Modifier.padding(top = 0.dp, bottom = 0.dp).onKeyEvent {
            when (it.key.keyCode) {
        ...
        }.focusRequester(focusRequester).focusable(), horizontalAlignment = Alignment.CenterHorizontally) {
