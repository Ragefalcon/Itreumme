...
fun Double.roundToString(round:Int):String  {
    ...
    val str = "${por.roundToInt()}${if (afterPoint.toDouble() == por && afterPoint.toDouble() != this%1*por) afterPoint - 1 else afterPoint}"
...
fun Double.roundToStringProb(round:Int):String  {
    ...
    val str = "${por.roundToInt()}${if (afterPoint.toDouble() == por && afterPoint.toDouble() != number%1*por) afterPoint - 1 else afterPoint}"
    val str2 ="${(number-number%1).roundToInt()}.${str.subSequence(str.length-round,str.length)}"
    val beforeL = str2.substringBefore(".").length
    val afterL = str2.substringAfter(".").length
    if (beforeL>3){
        for (i in 1 .. (beforeL-beforeL%3)/3) {
//        for (i in 1 .. if(beforeL%3 != 0) (beforeL-beforeL%3)/3 else (beforeL-beforeL%3)/3-1) {
